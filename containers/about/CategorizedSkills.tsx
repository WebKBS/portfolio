...
import { SkillCategory, SkillItem } from '@/portfolio-data/aboutSkillsData';
...
const SkillCard = ({ skill }: { skill: SkillItem }) => {
  return (
    <div className='group relative overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-gray-700 dark:bg-gray-600'>
      {/* Level badge in top-right corner */}
      {skill.level && (
        <span
          className={`absolute top-2 right-2 rounded-full px-2 py-0.5 text-xs font-medium text-white ${getLevelColor(skill.level)}`}
        >
      ...
      <div className='flex flex-col items-center gap-3'>
        ...
        <div className='text-center'>
          <h3 className='text-sm font-semibold text-gray-900 dark:text-gray-100'>{skill.title}</h3>
        </div>
      </div>
      ...
      <div className='absolute inset-0 flex items-center justify-center bg-gradient-to-br from-blue-600/95 to-purple-600/95 p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100'>
        <p className='text-center text-xs leading-relaxed text-white'>{skill.description}</p>
      </div>
    </div>
  );
};
